const wordCategories = {
  "Old Things": [
    "Pocket Watch", "Model T Food", "Washboard", "Rotary Telephone", "Fountain Pen", "Ice Box", "Horse and Carriage", "Typewriter", "Floppy Disk", "VHS Tape", "Pager (or Beeper)", "Tamagotchi Pet", "Game Boy", "Dial-Up Modem", "Boombox", "Polaroid Camera", "Slap Bracelet", "Discman Player", "Slinky Toy", "Furby Robot"
  ],
  "Fad Flavors": [
    "Hot Honey", "Birthday Cake", "Pickle", "Mango Habanero", "Pumpkin Spice", "Cookie Butter", "Black Garlic", "Blue Raspberry", "Maple", "Cotton Candy", "Gochujang (BBQ)", "Yuzu", "Matcha (Green Tea)", "S’more", "Chipotle", "Ube", "Bubblegum", "Thai Chili", "Chimichurri", "Rose", "Lavender", "Saffron", "Dragonfruit", "Brown Butter"
  ],
  "Pro Sports Teams with Curses": [
    "Boston Red Sox", "Chicago Cubs", "Cleveland Guardians", "Detroit Lions", "Toronto Maple Leafs", "Arizona Cardinals", "Sacramento Kings", "Atlanta Hawks", "Cleveland Browns", "Buffalo Bills", "Atlanta Braves", "Minnesota Vikings", "Chicago White Sox", "Texas Rangers", "San Francisco Giants", "Milwaukee Brewers", "Los Angeles Clippers", "Seattle Mariners", "Pittsburgh Pirates", "Washington Wizards", "New York Jets", "New York Knicks", "Buffalo Sabres", "Tennessee Titans", "Philadelphia Eagles", "Washington Commanders", "Denver Broncos", "Miami Dolphins", "Baltimore Orioles", "Cincinnati Reds"
  ],
  "Unusual USA Holidays": [
    "Fruitcake Toss Day (January 3)",
    "Squirrel Appreciation Day (January 21)",
    "National Hugging Day (January 21)",
    "Opposite Day (January 25)",
    "National Kazoo Day (January 28)",
    "Bubble Wrap Appreciation Day (Last Monday in January)",
    "Eat Ice Cream for Breakfast Day (First Saturday in February)",
    "National Toothache Day (February 9)",
    "Tell a Lie Day (April 4)",
    "National High Five Day (Third Thursday in April)",
    "Star Wars Day (May 4)",
    "No Socks Day (May 8)",
    "Dance Like a Chicken Day (May 14)",
    "National Pizza Party Day (Third Friday in May)",
    "National Frankenstein Day (August 30)",
    "Talk Like a Pirate Day (September 19)",
    "International Ninja Day (December 5)",
    "Festivus (December 23)"
  ],
  "Items You'd Steal From DAISO": [
    "Tape", "Gel pens", "Sticky notes", "Mechanical pencils", "Erasers", "Index cards", "Gift bags", "Paper clips", "Magnets", "Keychains", "Hair clips", "Lint rollers", "Stuffed animals", "Chopsticks", "Snack bags", "Storage bins", "Clothes hangers", "Socks", "Towels", "Toothbrush holders", "Cleaning wipes", "Faux plants", "Picture frames", "Mochi snacks", "Tea packets"
  ],
  "Things I Can't Put On Cereal": [
    "Ketchup", "Mustard", "Mayonnaise", "Relish", "Hot sauce", "BBQ sauce", "Soy sauce", "Whipped cream", "Worcestershire sauce", "Sriracha", "Vinegar", "Olive oil", "Ranch dressing", "Caesar dressing", "Horseradish", "Hummus", "Salsa", "Guacamole", "Pesto", "Honey mustard", "Chili oil", "Fish sauce", "Wasabi", "Dijon mustard"
  ],
  "Weird Pet Animals": [
    "Fox", "Axolotl", "Hedgehog", "Tarantula", "Gecko", "Beetle", "Miniature Pig", "Hermit Crab", "Miniature Goat", "Ferret", "Falcon", "Iguana", "Snake", "Duck", "Chicken", "Gerbil", "Guinea Pig", "Ocelot", "Macaw (Parrot)", "Pacman Frog", "Bat"
  ]
};
